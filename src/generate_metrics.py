...
for k in range(31):
    if k == 0:
        ...
        precision_K.append(
            [
                k,
                precision_at_k(resnet50_all[:, :k], k),
                precision_at_k(resnet101_all[:, :k], k),
                precision_at_k(vgg_all[:, :k], k),
            ]
        )
        reciprocal.append(
            [
                k,
                mean_reciprocal_rank(resnet50_all[:, :k]),
                mean_reciprocal_rank(resnet101_all[:, :k]),
                mean_reciprocal_rank(vgg_all[:, :k]),
            ]
        )
print(precision_K)
print(reciprocal)
pd.DataFrame(precision_K, columns=["k", "resnet50", "resnet101", "vgg16"]).to_csv(
    "precision_at_K.csv", index=False
)
pd.DataFrame(reciprocal, columns=["k", "resnet50", "resnet101", "vgg16"]).to_csv(
    "mean_reciprocal_rank.csv", index=False
)
